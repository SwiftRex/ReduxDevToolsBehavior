...
enum RemoteDevOutbound {
    ...
    func toJSON(describeAction: Convert<Any, (typePath: String, payloadJSON: String), Never>) -> String {
        switch self {
        ...
        case let .action(originalAction, state, instanceId):
            let (typePath, payloadJSON) = describeAction.run(originalAction).value
